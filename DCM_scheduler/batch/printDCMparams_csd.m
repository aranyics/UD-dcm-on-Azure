function printDCMparams_csd(DCM, data_DCM_path, modelName, SubjName, resting_state)
    ...
    dlmwrite(csvname, full(DCM.Ep.transit)', '-append', 'delimiter', ',');
    csvname = strcat(data_DCM_path,'/results/csv/',modelName,'/',SubjName,'_',modelName,'_ptransit.csv'); copyfile('hd.csv', csvname);
    dlmwrite(csvname, full(DCM.Pp.transit)', '-append', 'delimiter', ',');
    
    csvname = strcat(data_DCM_path,'/results/csv/',modelName,'/',SubjName,'_',modelName,'_decay.csv'); copyfile('hd.csv', csvname);
    dlmwrite(csvname, full(DCM.Ep.decay)', '-append', 'delimiter', ',');
    csvname = strcat(data_DCM_path,'/results/csv/',modelName,'/',SubjName,'_',modelName,'_pdecay.csv'); copyfile('hd.csv', csvname);
    dlmwrite(csvname, full(DCM.Pp.decay)', '-append', 'delimiter', ',');
    
    csvwrite( strcat(data_DCM_path,'/results/csv/',modelName,'/',SubjName,'_',modelName,'_epsilon.csv'), full(DCM.Ep.epsilon) );
    csvwrite( strcat(data_DCM_path,'/results/csv/',modelName,'/',SubjName,'_',modelName,'_pepsilon.csv'), full(DCM.Pp.epsilon) );
