...
def main(model):
    #print(model)
    dbFile = "/mnt/raid6_data/user/dszabo/azure/scripts/uploader/uploader_" + model + "_db.csv"
    ...
    paths = ["/mnt/raid6_data/hc/azure/analysis/DCM/largescale/conn_4sess"]
    ...
    for path in paths:
        ...
        for subj in subjects:
            sess = "/" + "_".join(model.split("_")[0:2])
            source = path+"/"+subj+sess
            #print(source)
            ...
            #print(res[0])
            #print("----")
            out = os.popen("/mnt/raid6_data/user/dszabo/azure/scripts/uploader/copy_to_azure.sh %s %s %s" % (source, res[0], ip)).read()
            print(out)
            db[subj] = ip
